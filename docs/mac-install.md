...
- 首次启动服务时需要联网，用于准备项目运行依赖。

安装包内置 Node.js/npm、uv、ffmpeg/ffprobe 等基础运行工具；用户不需要先在终端安装这些工具。
...
课程资料默认保存在同一目录下的 `Workspace`。你也可以在 App 中改到其他稳定位置。

第一次启动服务时，Course Navigator 会用内置运行工具准备本地运行依赖，需要联网，可能需要几分钟。之后再次启动会更快。
