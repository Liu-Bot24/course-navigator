...
- 建立课程库，支持专辑分组、标题编辑、排序、本地视频导入和视频缓存管理。
- 导入或导出轻量课程包，分享课程、校正字幕、翻译字幕和 AI 学习材料。
- 通过 `yt-dlp` 提取平台字幕，也支持本地 ASR、在线 ASR 和本地字幕上传。
- 用独立 Workspace 管理课程记录、AI 学习材料和本地视频文件；字幕提取与设置等运行数据留在本地运行目录。
- 在线 ASR 支持 xAI、OpenAI Whisper、Groq Whisper 和自定义兼容接口。
- 对需要登录的视频，可使用直接访问、浏览器登录状态或 cookies 文件。
- 支持视频与字幕联动、点击时间戳跳转、双语字幕视图和时间戳导航。
- 生成导览、大纲、解读和详解，支持标准 / 高保真详细程度。
- 导览提供初学 / 进阶学习建议，帮助判断重点内容和可略读内容。
- 翻译字幕和标题。
- 在 ASR 校正工作台中审阅自动识别字幕，支持前后对照、悬停理由、置信度、接受/拒绝、批量接受和自动保存。
- 在 ASR 校正前补充术语、人名、产品名和常见误识别。
- 可选使用 Tavily 或 Firecrawl 校验 ASR 修改候选项。
- 翻译、课程分析和 ASR 校正共用模型档案库，支持 OpenAI 兼容格式和 Anthropic 格式。

## 最近更新

- 学习地图：新增标准 / 高保真详细程度；导览补充初学和进阶学习建议，帮助判断重点、略读和复习内容。
- 本地视频与 Workspace：支持顶部导入本地视频，课程资料集中保存在 Workspace；删除本地导入课程会同步删除对应视频。
- 字幕来源与 ASR：支持本地字幕上传、在线 ASR、本地 ASR 进度显示和中文简体化；在线 ASR 可选 xAI、OpenAI Whisper、Groq Whisper 或自定义接口。
- 课程包分享：支持导出单课或完整专辑，保留校正字幕、翻译字幕、AI 学习材料、留言、专辑名称和课程顺序。
- ASR 校正：支持搜索校验、键盘切换建议、Enter 接受、悬停理由卡、置信度排序、一键接受高置信度建议和接受后自动保存。
- 课程库细节：支持专辑排序、删除专辑，并在删除课程时清理相关本地缓存。
...
Course Navigator 会把课程记录、学习材料、导入视频和本地视频缓存保存在课程资料 Workspace 中。字幕提取、ASR 中间文件和本地设置保存在本地运行数据目录中。首次使用新版 Workspace 时，应用会把旧数据目录里的课程记录和本地视频缓存复制到 Workspace。
