...
Course Navigator 是一个视频课程学习工作台。你可以粘贴支持的视频链接或导入本地视频，提取字幕，在视频旁边校阅逐字稿，把课程整理成专辑，并用 AI 完成字幕翻译、课程分析和 ASR 字幕校正。
...
- 建立课程库，支持专辑分组、课程标题、排序、本地视频导入和视频缓存管理。
- 导入和导出轻量课程包，用于分享已整理好的课程、校正后的字幕、翻译字幕和 AI 学习材料。
- 通过 `yt-dlp` 从支持的视频页面提取字幕，也可以使用本地 ASR、在线 ASR 或本地上传字幕文件。
- 使用独立 Workspace 管理正在使用的课程资料，包括课程记录、AI 学习材料和本地视频文件；字幕提取和设置等运行数据留在本地运行目录。
...
- 本地视频导入：可以直接从顶部工具栏导入本地视频，课程会保存到 Workspace，并自动进入本地播放模式。导入的视频可以使用本地 ASR 或在线 ASR 生成字幕。
- Workspace 管理：课程资料现在集中保存在独立 Workspace 中；课程记录、AI 学习材料和正在使用的视频文件会随课程一起管理，字幕提取过程文件和程序设置继续保存在本地运行目录。
- 本地视频课程清理：本地导入课程不再提供单独的“移除缓存”入口。删除课程时会同步删除课程记录和导入的视频文件；普通在线课程的下载缓存仍可单独移除。
...
课程库可以按专辑管理视频，编辑课程和专辑名称，调整课程顺序，复制来源链接，删除课程记录，并管理在线课程的本地视频缓存。对于导入的本地视频，视频文件是课程资料的一部分；删除课程会同步删除这份视频文件。专辑可以用于课程列表、讲座系列、访谈合集，或者任何长期学习项目。
...
Course Navigator 会把课程记录、学习材料、导入视频和本地视频缓存保存在你的课程资料 Workspace 中。字幕提取、ASR 中间文件和本地设置保存在本地运行数据目录中。首次使用新版 Workspace 时，应用会把旧数据目录里的课程记录和本地视频缓存复制到 Workspace。
